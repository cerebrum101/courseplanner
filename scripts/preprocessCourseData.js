...
function extractCourseCodesFromString(reqString) {
  if (!reqString) return [];
  const matches = reqString.match(/%([^%]+)%/g) || [];
  return matches.map(match => match.slice(1, -1).trim());
}

function extractCourseCodesFromArray(reqArray) {
  if (!reqArray) return [];
  ...
  reqArray.forEach(item => {
    ...
    } else if (typeof item === 'object' && item.courses) {
...
function preprocessCourseData(inputFile, outputFile) {
